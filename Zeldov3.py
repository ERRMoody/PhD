...
for x in files: 
	...
	for record in SeqIO.parse (FILE, "phylip"):
		...
		file.write ('\n %s' %idx)
		id = str(idx)
		species_ogts[id_to_species[str(idx)]].append(OGT) 
		#print species_to_id['Bacillus_anthracis_str_Ames']
...
file.close()
f = open ("importantresults.txt", "w+")
for keys, values in species_ogts.items():
	po = (keys)
	lala = np.mean(values)
	dipsy.append (lala)
	f.write ("\n %s" %po)
...
f = open ("importantresults2.txt", "w+")

tinkywinky = np.mean(dipsy)
